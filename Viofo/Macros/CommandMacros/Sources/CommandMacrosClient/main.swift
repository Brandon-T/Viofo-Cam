...
struct ViofoFirmware: Equatable {
    ...
    static func from(_ firmware: String) throws -> ViofoFirmware {
        ...
        return ViofoFirmware(vendor: vendor, model: model, device: .a329, version: version, versionNumber: versionNumber, buildDate: date)
    }
}
...
let command = Command(for: .init(vendor: "", model: "A329", device: .a329T, version: "1.2", versionNumber: 1.2, buildDate: Date()))

if case .int(let v)? = command.resolve("BEEP_SOUND") {
    print(v) // 9403
}

if case .int(let v)? = command.resolve("FRONT_CAMERA_HDR") {
    print(v) // 9403
}
